...
students = sorted(list(students)) # преобразовали в список и отсортировали по алфавиту


vedom = {} # создаём пустой словарь

for i in range(len(students)):
   vedom[students[i]] = sum(grades[i])/len(grades[i]) # Вычисляем средний бал
